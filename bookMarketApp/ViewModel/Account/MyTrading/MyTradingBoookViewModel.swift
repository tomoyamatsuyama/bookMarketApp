...
class MyTradingBookViewModel: NSObject, UITableViewDataSource {
    public private(set) var tradingData = TradingData()
    
    func setTradingData(completion: (() -> Void)? = nil) {
        Api.Books.getTradingData(completion: { trading in
            self.tradingData = trading
            completion?()
        })
    }
